...
项目采用轻量的流水线（Pipeline）架构，并为外部 AI 失败设计降级路径，核心包含：
- **Event Watcher**：利用 Kubernetes `client-go` 的 `SharedInformer` 机制，低开销、实时地监听集群中所有命名空间的 `Warning` 事件。
- **Dedup Engine**：采用滑动窗口结合冷却周期（Cooldown）算法，对同一指纹（Fingerprint）的事件进行聚合与抑制。实际降噪比例取决于事件分布，后续将通过事件回放数据集量化。
...
### 1. 系统拓扑架构
![系统拓扑架构图](01-system-architecture.drawio.png)

### 2. 诊断流水线处理流程 (Pipeline Flow)
![流水线处理流程图](02-pipeline-flow.drawio.png)

---

## ☁️ 云原生部署实验 (Deployment & Delivery)

为了验证交付路径，本项目完成了容器化构建与 Serverless K8s 托管实验：

### 1. 云端 ACK/ASK 部署拓扑
![ACK部署拓扑架构](03-ack-deployment.drawio.png)

### 2. 阿里云 ACK 容器服务控制台集群概览
在阿里云上配置和运行的真实 Kubernetes 托管集群，右侧折线图展示了待治理的真实 Warning 事件趋势波形：
![阿里云 ACK 控制台集群健康状态与事件概览](image_copy_3.png)

### 3. 阿里云 ACR 镜像构建仓库
在阿里云容器镜像服务（ACR）中自动进行多版本构建与安全托管：
![ACR控制台版本列表](image_copy_2.png)
...
- **云端控制台运行实机**：
  ![ECI运行状态列表](image_copy_4.png)
- **本地终端 kubectl get pods -A 状态排查**：
  ![终端 kubectl 命令行查看 Pods 运行详情](image_copy.png)
...
- **数据模型设计**：
  ![数据模型设计图](04-data-model.drawio.png)
- **有界并发模型**：基于 Go 的 `channel` 和 `worker pool` 实现异步处理并限制并发；吞吐上限仍需通过基准测试给出。
- **降级保护 (Fallback)**：当外部 AI 接口超时或解析异常时，流水线退回静态规则模板，避免单个外部依赖阻塞核心通知路径。
- **RBAC 最小权限原则**：K8s YAML 清单中配置了最细粒度的 Read-Only ClusterRole，仅授予 Pods 和 Events 的 Get/List/Watch 权限，防止特权提升。

项目源码：[user27c/Kube-AI-Diagnoser](https://github.com/user27c/Kube-AI-Diagnoser)
